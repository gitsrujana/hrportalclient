...
import { Modal, Box, Typography, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, useMediaQuery } from '@mui/material';
...
const EmployeeAttendanceDetails = ({ email, attendanceData, onClose }) => {
  ...
  const employeeAttendance = attendanceData.filter(att => att.email === email);
    ...
    <>
      ...
      >
        <Box sx={{
          ...
          <CloseIcon onClick={onClose} sx={{ color: '#000', marginLeft: "100%", color: "red" }} />
          ...
          <Box sx={{ marginTop: 2 }}>
            {/* <Typography  sx={{ textAlign: { xs: 'center', sm: 'left',fontSize: isMobile ? "12px" : "16px",
                fontWeight: "bold",
                color: "#d32f2f",  } }}>
